...
OMEGA_d = -1.5*sqrt(mu)*J2*R_earth^2/(1-e^2)^2/a^(7/2) * cos(i0);
% OMEGA_d = -2.3394e-7
% omega_d = -1.5*sqrt(mu)*J2*R_earth^2/(1-e^2)^2/a^(7/2) * (5/2*sind(i0)^2-2)
omega_d = OMEGA_d * (5/2*sin(i0)^2-2)/cos(i0);
...
for n_time=1:N_points
    ...
    t = t0 + dt;
    M = 2*pi*t/T;
    E = newton_raphson(@(E) E-e*sin(E)-M, @(E) 1-e*cos(E), 0.3); % this might need fixing for E to become +ve
