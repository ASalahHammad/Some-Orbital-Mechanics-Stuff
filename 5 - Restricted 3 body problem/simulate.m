function [] = simulate(t, dt, mass_vec, r, v, a, styles, legends, ax, obj)

G = 6.67e-11;

N_bodies = int32(size(r, 1) / 3);

% dt is the jump in the tme stamps, it's not the actual delta_time that you
% use to solve ODEs
if(~exist("styles", "var"))
    styles = ["r", "g", "b"];
end
...
pp11 = quiver3(ax, nan, nan, nan, nan, nan, nan);
pp12 = quiver3(ax, nan, nan, nan, nan, nan, nan);
qq = quiver3(ax, nan, nan, nan, nan, nan, nan);

n_vec = 1:dt:length(t);
if(ismember(t(end), n_vec))
    n_vec = [n_vec, t(end)];
end
for n = n_vec % time loop   
    delete(qq); delete(pp1); delete(pp2); delete(pp3); delete(pp4); delete(pp5); delete(pp6); delete(pp7); delete(pp8); delete(pp9); delete(pp10); delete(pp11); delete(pp12);
    for k = 1:N_bodies
        ind_x = (k-1)*3+1;
        ind_y = (k-1)*3+2;
        ind_z = (k-1)*3+3;
    ...
    x_vec = [linspace(xlims(1),-3e8,40), linspace(-3e8,3.5e8,80), linspace(3.5e8,xlims(2),40)];
    x_vec = unique(x_vec);
    % x_vec = linspace(xlims(1),xlims(2),150);
    y_vec = [linspace(ylims(1),-3.5e8,40), linspace(-3.5e8,3.5e8,80), linspace(3.5e8,ylims(2),40)];
    y_vec = unique(y_vec);
    % y_vec = linspace(ylims(1),ylims(2),100);
    ...
    for i = 1:size(xGrid,1)
	    for j = 1:size(xGrid,2)
		    X = xGrid(i,j);
		    Y = yGrid(i,j);
		    R_i = sqrt((X-r(1:3:end,n)).^2 + (Y-r(2:3:end,n)).^2);
            d = norm(r(1:3) - r(4:6));
            mu = G*(sum(mass_vec));
            omega = sqrt(mu/d^3);
		    PHI(i,j) = sum( -G*mass_vec'./R_i ) - 0.5*omega^2*(X^2+Y^2); % Gravity Potential = Phi_gravity + Phi_centrifugal
            % PHI(i,j) = sum( -G*mass_vec'./R_i );
	    end
    end
    [~,pp11] = contour(ax, xGrid, yGrid, PHI, 15000, 'k');
    % pp12 = plot(xGrid,yGrid,'ko');
    pause(0.001);
end
